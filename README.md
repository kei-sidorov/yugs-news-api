...
Метод должен отдавать список новостей из хранилища по ID типа $type с заданным лимитом $limit и страницей $page (нумерация с единицы) в формате массива. Массив должен содержат ассоциативные массивы со следующими ключами:
...
Все настройки вынесенный в INI файл. Файл располагается по пути config/config.ini.

Для запуска приложения необходимо внести несколько исправлений.

1. В секции **[database]** поправить доступы к базе данных MySQL.
2. В секции **[global]** указать вложенность **app-level** и путь до API **path** относительно Document Root
3. В секции **[news]** необходимо указать класс **class** созданного ранее класса коллекции новостей (в нашем случае MyNewsCollection)
