...
* devId - MD5 хеш от уникального идентификатора устройства, в основном используется для органичений на показ и таргетинга
...
**Ответ скрипта должен быть в формате *JSON* и иметь соответсвующий заголовок Content-type.**
...
* [height] - высота баннера в писель-незваисимых точках

Картинка баннера должна быть в формате PNG, JPG или неанимированный GIF. Стоит принять во внимание, что плотность пискселей на современных мобильных устройсвах выше чем 1 писксель на точку, выбирайте разрешение картинки исходя из этого факта.

Ссылка на картинку, указанная в параметре destination по умолчанию открывается c помощью ствроенного вприложение барузера. Если нужно открыть ссылку во внешем системном браузере укажите **#external** в конце ссылки.

Интервал времени в секундах до следующего запроса баннера имеет минимальное значение, раньше которого не произойдет слудующий запрос. Как писалось ранее, баннера запрашиваются при запросе списка новостей, при условии что прошло времени не меньше, чем было указанно в предидущем запросе баннера.
 
Значения высоты и ширины являются независмыми от физического разрешения экрана, и указанны в точках. Параметры не обязательные и по умолчанию имеют размеры 250&times;400 точек. Используя эти параметры вы можете показывать разные баннера для планшетов и телефонов. Если значение высоты или ширины в ответе скрипта окажутся больше чем таковые у устройства, баннер будет проигнорирован приложением.

#### Bottom баннера

Это баннера в нижней части экрана. Могут быть двух вариантов: HTML5 баннера или баннер из системы Google AdMob. Их возможно чередовтвать, например когда рекламное место не продано показывать AdMob, в ином случае собсвенные баннера.  
...
* type - тип баннера, должен быть равер AdMob
* admob_id - идентификатор в системе google AdMob

##### HTML

* type - тип баннера, долже быть равен html
...
При использовании HTML баннера нужно реализовать минимальные требования к баннеру. При запросе приложение дописывает к url значение размера баннера, которое зависит от размера экрана устройства в формате #[ширина]x[высота]. Высота зависит от ширины устройсва:

* 32 точки, если ширина экрана менее 320 точек
* 50 точек, если ширина >= 320, но меньше 700
* 90 точек, если ширина >= 700 точек

С помошью параметра разработчик баннера должен организвать адаптивность и не допустить скрол.

**Внимание** Приложение не покажет баннер пока не будет инициораван переход по ссылке banner:ready (window.location = 'banner:ready'). Желаетельно инициировать этот переход через Javascript при полной загрузке ресурсов вебстраницы баннера, на событии window load.
